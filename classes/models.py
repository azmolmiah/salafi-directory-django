...
class Class(models.Model):
    ...
    sub_Heading = models.CharField(max_length=30, blank=True, help_text='Max 30 characters')
    ...
    photo = models.ImageField(upload_to='classes/', default='classes/', help_text='Must be a height, width of 200px and image size under 20 Kilobytes.')
    ...
    def clean(self):
        ...
        if(self.photo.file.size > 20000):
             raise ValidationError(f'The image is {self.photo.file.size / 1000} Kilobytes. Must be 20 Kilobytes or less.')
    
        w, h = get_image_dimensions(self.photo)
        if w != 200:
            raise ValidationError("The image is %i pixel wide. Must be 200px." % w)
        if h != 200:
            raise ValidationError("The image is %i pixel high. Must be 200px." % h)
